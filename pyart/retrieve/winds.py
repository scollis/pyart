...
def _radar_coverage(grids, refl_field=None, vel_field=None):
    ...
    return {'data': cover.astype('int32'),
...
def _column_types(cover, base, fill_value=None):
    ...
    cover = np.asfortranarray(cover['data'], dtype='int32')
    base = np.asfortranarray(base['data'], dtype='float64')
    
    column = qc.column_type(cover, base, fill_value=fill_value)
    
    # Return dictionary of results
    
    comment = '0 = Undefined, 1 = Well-defined, 2 = Top-defined ' + \
              '3 = Anvil-like, 4 = Transition-like, 5 = Discontinuous'
...
def _arm_merge_sonde(grid, sonde, target, fill_value=None, standard=False,
                     finite_scheme='basic', rho0=1.2, H=10000.0, debug=False,
                     verbose=False):
    """
    Parameters
    ----------
    ...
    drho : np.ndarray
        ...
        in kg m^-4
    u, v : np.ndarray
        Eastward and northward wind component profiles in m s^-1
    """
    ...
    # Get axes
    #
    # We have to remember that the heights given in the merged sounding 
    ...
    if standard:
        ...
        # Define the specific gas constant of dry air
        
        R = 287.058 # in (J K^-1 mol^1)
        
        rho = (P * 1000.0) / (R * (T + 273.15)) # in (kg m^-3)
        
        drho = pyart.retrieve.continuity.density_gradient(rho, z_sonde,
                        fill_value=fill_value, finite_scheme=finite_scheme)
...
def _fall_speed_caya(network, T, fill_value=None, proc=1, refl_field=None):
    ...
    ze = deepcopy(network.fields[refl_field]['data'])
    ze = np.ma.filled(ze, fill_value)
    ze = np.asfortranarray(ze, dtype='float64')
...
def solve_wind_field(grids, network, sonde, target, dx=500.0, dy=500.0,
    ...
    #
    # We will eventually have to permute the problem from the initial grid
    # space which is in (nz,ny,nx) to a vector space, i.e. 1-D
    #
    ...
    # scale, if necessary
    #
    # The length scale is designed to make the dimensionality of each cost
    # uniform, as well as bring each cost within 1-3 orders of magnitude of
    # each other
    ...
    # Use the ARM merged sounding product to get the atmospheric
    # thermodynamic and horizontal wind profiles
    
    T, P, rho, drho, us, vs = _arm_merge_sonde(grids[0], sonde, target,
                                    fill_value=fill_value, standard=True,
    ...
    if first_guess == 'zero':
        ...
        u0 = np.zeros(N, dtype='float64')
        v0 = np.zeros(N, dtype='float64')
        w0 = np.zeros(N, dtype='float64')
    ...
    if background == 'zero':
        ...
        ub = np.zeros((nz,ny,nx), dtype='float64')
        vb = np.zeros((nz,ny,nx), dtype='float64')
        wb = np.zeros((nz,ny,nx), dtype='float64')
    ...
    if use_qc:
        ...
        grids = _radar_qc(grids, mds=mds, ncp_min=ncp_min, rhv_min=rhv_min,
    ...
    #
    # We also want to make sure that the fall speed data array is a NumPy
    # array and not a NumPy masked array
    ...
    #
    # We will also make sure that the echo base and top arrays are NumPy
    # arrays and not NumPy masked arrays
    ...
    #
    # We need to make sure that the velocity fields for every grid are
    # represented by NumPy arrays and not NumPy masked arrays
    ...
    if technique == '3d-var' and 'scipy' in solver:
        ...
        # This is an important step
        #
        # Here we get the function handle for the appropriate SciPy solver
        ...
        if solver == 'scipy.fmin_cg':
            ...
            # SciPy Broyden-Fletcher-Goldfarb-Shanno method
            #
            # Using this algorithm requires storage of a 3N x 3N matrix
            # (the Hessian matrix), and in most wind retrieval applications
            # will throw a MemoryError
        ...
        #
        # Group the required arguments for the cost function and gradient
        # together. The order at which the arguments are entered is very
        # important, and must be adhered to by the functions that use it
        # as arguments
        ...
        # Check if the user wants to perform a first pass which
        # retrieves a heavily-smoothed horizontal wind field and
        # a w field of 0 everywhere
        ...
        if first_pass:
            ...
            # This is an important step
            #
            # We have to change the arguments to account for different
            # costs and weighting coefficients for the first pass
            
            args0 = (nx, ny, nz, N, grids, ub, vb, wb, vt['data'], rho, drho,
                    base['data'], top['data'], column['data'], wgt_o, wgt_c,
                    wgt_s0, wgt_b, wgt_w0, continuity, smooth_cost, dx, dy,
                    dz, sub_beam, finite_scheme, fill_value, proc, vel_field,
                    debug, verbose)
