...
def _cost_wind(x, *args):
    ...
    if continuity_cost is None:
        ...
        div, du, dv, dw = divergence.full_wind(
            u, v, w, dx=dx, dy=dy, dz=dz, finite_scheme=finite_scheme,
            fill_value=fill_value, proc=proc)

        # Calculate the anelastic continuity cost Jc
        Jc = continuity.wind_cost_potvin(
            w, du, dv, dw, rho, drho, wgt_c=wgt_c, fill_value=fill_value)

    elif continuity_cost in ['Protat', 'bottom-up', 'top-down']:
        ...
        div, du, dv = divergence.horizontal_wind(
            u, v, dx=dx, dy=dy, finite_scheme=finite_scheme,
            fill_value=fill_value, proc=proc)
        ...
        if continuity_cost == 'Protat':
            ...
            wu = continuity.integrate_up(
                div, rho, drho, dz=dz, fill_value=fill_value)
            wd = continuity.integrate_down(
                div, echo_top, rho, drho, z_disp, dz=dz, fill_value=fill_value)
            wc = continuity.weight_protat(
                wu, wd, echo_top, z_disp, fill_value=fill_value)

        elif continuity_cost == 'bottom-up':
            # Explicitly integrate the anelastic air mass continuity
            # equation upwards starting from the surface
            wc = continuity.integrate_up(
                div, rho, drho, dz=dz, fill_value=fill_value)

        else:
            # Explicitly integrate the anelastic air mass continuity
            # equation downwards starting from the echo top
            wc = continuity.integrate_down(
                div, echo_top, rho, drho, z_disp, dz=dz, fill_value=fill_value)

        # Calculate the anelastic continuity cost Jc
        Jc = continuity.wind_cost_iterative(
            w, wc, wgt_c=wgt_c, fill_value=fill_value)
    ...
    if smooth_cost is None:
        ...
        # the Fortran routine
        res = laplace.full_wind(
            u, v, w, dx=dx, dy=dy, dz=dz, proc=proc,
        ...
        Js = smooth.wind_cost_potvin(
            dux, duy, duz, dvx, dvy, dvz, dwx, dwy, dwz, wgt_s1=wgt_s1,
            wgt_s2=wgt_s2, wgt_s3=wgt_s3, wgt_s4=wgt_s4, fill_value=fill_value)
    ...
    if ub is None or vb is None or wb is None:
        ...
        Jb = background.wind_cost(
            u, v, w, ub, vb, wb, wgt_ub=wgt_ub, wgt_vb=wgt_vb, wgt_wb=wgt_wb,
            fill_value=fill_value, proc=proc)
...
def _grad_wind(x, *args):
    ...
    if continuity_cost is None:
        ...
        # unpack these after
        res = continuity.wind_gradient_potvin(
            w, du, dv, dw, rho, drho, wgt_c=wgt_c, dx=dx, dy=dy, dz=dz,
            finite_scheme=finite_scheme, fill_value=fill_value)
        dJcu, dJcv, dJcw = res
        ...
        if continuity_cost == 'Protat':
            ...
            wu = continuity.integrate_up(
                div, rho, drho, dz=dz, fill_value=fill_value)
            wd = continuity.integrate_down(
                div, top, rho, drho, z_disp, dz=dz, fill_value=fill_value)
            wc = continuity.weight_protat(
                wu, wd, top, z_disp, fill_value=fill_value)
    ...
    if smooth_cost is None:
        ...
        res = smooth.wind_gradient_potvin(
            dux, duy, duz, dvx, dvy, dvz, dwx, dwy, dwz, wgt_s1=wgt_s1,
            wgt_s2=wgt_s2, wgt_s3=wgt_s3, wgt_s4=wgt_s4, dx=dx, dy=dy, dz=dz,
            finite_scheme=finite_scheme, fill_value=fill_value)
        dJsu, dJsv, dJsw = res
...
def _radar_qc(grids, mds=0.0, vel_max=55.0, vel_grad_max=10.0, ncp_min=0.3,
    ...
    for grid in grids:
        ...
        is_bad_vel_grad = np.logical_or(
            grad_mag > vel_grad_max, grad_mag == fill_value)
...
def _arm_interp_sonde(grid, sonde, target=None, rho0=1.2, H=10000.0,
    ...
    z_grid = grid.axes['z_disp']['data'] + grid.axes['alt']['data']
    z_sonde = 1000.0 * sonde.variables['height'][:]
    ...
    # Temperature should be in degrees Celsius, pressure should be in kPa,
    # and horizontal wind components should be in m/s
    temp = sonde.variables['temp'][t,:]
    pres = sonde.variables['bar_pres'][t,:]
    u = sonde.variables['u_wind'][t,:]
    v = sonde.variables['v_wind'][t,:]
    ...
    if standard_density:
        ...
        rho = (pres * 1000.0) / (R * (temp + 273.15))  # in (kg m^-3)
        drho = gradient.density1d(
            rho, z_sonde, fill_value=fill_value, finite_scheme=finite_scheme)
...
def _check_analysis(grids, conv, sonde=None, target=None, fall_speed='Caya',
    ...
    res = _arm_interp_sonde(
        grids[0], sonde, target=target, standard_density=standard_density,
        fill_value=fill_value, debug=False, verbose=verbose)
...
def solve_wind_field(
        grids, sonde=None, target=None, technique='3d-var',
        ...
        impermeability='strong', first_pass=True, sub_beam=False, wgt_o=1.0,
        wgt_c=1.0, wgt_s1=1.0, wgt_s2=1.0, wgt_s3=1.0, wgt_s4=1.0, wgt_ub=1.0,
        wgt_vb=1.0, wgt_wb=1.0, wgt_w0=1.0, gtol=1.0e-1, ftol=1.0e7,
        maxiter=100, maxcor=10, disp=False, maxiter_first_pass=50,
        length_scale=None, use_qc=True, mds=0.0, ncp_min=0.4, rhv_min=0.8,
        vel_max=40.0, vel_grad_max=10.0, noise_ratio=50.0, window_size=10,
        min_layer=1500.0, top_offset=500.0, standard_density=False,
        save_refl=True, proc=1, proj='lcc', datum='NAD83', ellps='GRS80',
        fill_value=None, refl_field=None, vel_field=None, ncp_field=None,
        rhv_field=None, u_field=None, v_field=None, w_field=None, debug=False,
        verbose=False):
    """
    Parameters
    ----------
    ...
    if use_qc:
        _radar_qc(
            grids, mds=mds, vel_max=vel_max, vel_grad_max=vel_grad_max,
            ncp_min=ncp_min, rhv_min=rhv_min, window_size=window_size,
            noise_ratio=noise_ratio, refl_field=refl_field,
            vel_field=vel_field, ncp_field=ncp_field, rhv_field=rhv_field)
    ...
    if fall_speed == 'Caya':
        _fall_speed_caya(
            grids, temp, fill_value=fill_value, refl_field=refl_field)
    else:
    ...
    # conditions
    if continuity_cost in ['Protat', 'bottom-up', 'top-down']:
        base, top = _echo_bounds(
            grids, mds=mds, min_layer=min_layer, top_offset=top_offset,
            fill_value=fill_value, refl_field=refl_field)
    ...
    if technique == '3d-var' and 'scipy' in solver:
        ...
        args = (
            N, nx, ny, nz, dx, dy, dz, grids_no_mask, ub, vb, wb, rho, drho,
            wgt_c, wgt_s1, wgt_s2, wgt_s3, wgt_s4, wgt_ub, wgt_vb, wgt_wb,
            wgt_w0, continuity_cost, smooth_cost, impermeability,
            finite_scheme, echo_base, echo_top, column_type, sub_beam,
        ...
        if first_pass:
            ...
            args0 = (
                N, nx, ny, nz, dx, dy, dz, grids_no_mask, ub, vb, wb, rho,
                drho, wgt_c, wgt_s1_first, wgt_s2_first, wgt_s3_first,
                wgt_s4_first, wgt_ub, wgt_vb, wgt_wb, wgt_w0,
                continuity_cost_first, smooth_cost, impermeability,
                finite_scheme, echo_base, echo_top, column_type, sub_beam,
                fill_value, proc, vel_field, debug, verbose)
